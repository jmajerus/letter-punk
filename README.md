...
- "Set Board" (button lives in Settings, opens its own dialog) lets you paste a board from classic Letter Boxed, or generate one from your own solution word(s) — type a single word as a seed and Letter Punk finds a companion automatically. That same dialog's "Copy Blank Link" / "Copy Progress Link" buttons send a board to a friend as a URL — either a fresh challenge, or exactly where your own play currently stands, replaying pipe-by-pipe when they open it. If your progress was played in Free Chain mode, the recipient's copy turns it on automatically to match, without changing their own Settings default.
