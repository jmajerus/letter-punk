...
**No-overlap style recognition.** Grew directly out of the board's Strava-style route markers (a green dot where each word begins, a red dot where each completed word ends — see [docs/development.md](development.md)): a player could in principle solve a Free Chain puzzle such that no tile ever serves as both a red and a green — every word genuinely independent, no letter doing double duty as one word's ending and another's beginning. That's a real, distinct thing from Free Chain mode itself, which only *removes* the requirement to chain; nothing today stops or notices a player reusing a letter as both roles anyway.
...
Leaning toward option 2 for the same reason Two-Word-Only mode (above) sits here as a candidate rather than something to build reflexively: it fits the existing rating-title pattern without adding the game's first hard rejection rule.
