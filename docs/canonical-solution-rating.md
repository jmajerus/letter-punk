...
The canonical word count itself is derived the same way canonical character count already is — `getActiveCanonicalWords().length` in `app.js`, wired through a `getCanonicalWordCount` engine option that mirrors `getCanonicalCharacterCount` exactly. It isn't hardcoded to 2, even though every current daily puzzle's canonical solution happens to be exactly two words (`pickBalancedCompanion` only ever searches for a single companion, so the auto-generated path is a two-word pair by construction — see the next section). A custom board built via New Game's "generate from words" already accepts more than two solution words today (`wordsFromSolutionInput`/`generateBoardFromSolutionWords` only require 2+, no upper bound), so a hardcoded 2 would have been actively wrong for that case now, not just inflexible for some hypothetical future one.
