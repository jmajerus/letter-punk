...
## Why Character Count, Not Word Count *Alone*

An easy alternative would have been to score by word count instead — fewest words wins, which is how NYT Letter Boxed frames "par," and in fact the *only* axis it rates a solve on. That axis was set aside as the sole comparison, for a reason specific to this game: word count can't see the thing character count was added to reward. A tight, repeat-free word pair and a long, double-letter-laden word pair can both be "two words" — identical on that axis, wildly different in what they actually demonstrate. Character count is sensitive to exactly the dimension word count is blind to, which is why it exists here at all.

That's an argument for *adding* character count, though — not for demoting word count to an afterthought. An earlier cut of this design did exactly that: every solve with a known canonical reference earned one of the three character-count titles below, regardless of how many words it took, which meant a masked share's "Bonus +N" fired for essentially every completed daily puzzle. A word that's supposed to signal something not everyone gets had quietly become something everyone got — and worse, it gave a player no reason to ever chase fewer words, since a rambling six-word solve could earn the exact same praise as a tight two-word one. See "Word Count Gates Character Count" below for the fix.

## Word Count Gates Character Count

Character count only gets to speak once word count has already cleared its own bar: the player's word count has to be at or under the canonical solution's word count before Dead Reckoner/Efficiency Engineer/Vocabulary Wrangler are allowed to fire at all, in either the in-game message or the masked share's Bonus count (`isWordCountAtOrUnderCanonical` in `gameLogic.js`). A solve that took more words than the reference still gets its facts reported, plainly, with an invitation to try again for fewer — no title, no Bonus contribution from that axis, but no shaming either.

This isn't the two-branch "you beat it or you didn't" design the "Winning in the Center and to Either Side" section below describes and then moves past for character count — it's deliberately asymmetric, and that's the point. Character count stays three-way neutral (under/at/over the canonical count are all equally legitimate, since double letters are meant to be first-class) because there's no competing design goal pulling it toward "fewer is always better." Word count doesn't get that same neutrality, because there *is* a competing goal: it's the one axis genre convention already has strong, pre-existing opinions about, and Letter Punk gains nothing by pretending otherwise.

The canonical word count itself is derived the same way canonical character count already is — `getActiveCanonicalWords().length` in `app.js`, wired through a `getCanonicalWordCount` engine option that mirrors `getCanonicalCharacterCount` exactly. It isn't hardcoded to 2, even though every current daily puzzle's canonical solution happens to be exactly two words (`pickBalancedCompanion` only ever searches for a single companion, so the auto-generated path is a two-word pair by construction — see the next section). A custom board built via Set Board's "generate from words" already accepts more than two solution words today (`wordsFromSolutionInput`/`generateBoardFromSolutionWords` only require 2+, no upper bound), so a hardcoded 2 would have been actively wrong for that case now, not just inflexible for some hypothetical future one.
...
Two more titles, **Solo Plumber** and **Union Plumber**, are orthogonal to all three of the above rather than further branches of this same character-count logic — they measure whether words handed off into each other via a shared start/end tile, not character efficiency, and either can combine with whichever of the three titles above the count already earned. They're each other's structural opposite (zero overlap vs. every word chained) and mutually exclusive for a 2+-word solve. See [docs/development.md](development.md#solo-plumber) and [docs/development.md](development.md#union-plumber) for what they actually check.
