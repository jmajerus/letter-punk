...
**`game_solved` carries the actual solution, not just a count.** `trackGameSolved` takes the engine's `foundWords` at the exact moment a puzzle is solved, reversed into solve order, and sends it as a fourth argument. This is deliberately captured once, at that single instant, rather than reconstructed later from the stream of individual `word_submit` events — `foundWords` only ever holds words still part of the current attempt (Undo Word removes an entry outright, it doesn't just flag it), so there's no risk of a tried-then-abandoned word ending up in what gets logged as "the solution." The admin dashboard's existing "Recent solves" table (`src/admin.js`) surfaces this directly as a `words` column, so a real solve can actually be read, not just counted.

**"Player solutions" in the Yesterday modal reuses that same captured word list, but doesn't read it back out of Analytics Engine.** AE is for aggregate dashboard queries, not per-request public reads, so `src/worker.js`'s `/api/event` handler additionally writes each `game_solved`'s words into a small, purpose-built KV pool (`SOLUTIONS_KV`, optional — same graceful-when-unbound pattern as `PUZZLES_KV`/`PSA_CACHE`), keyed `solutions:{date}` and capped (currently 20) and deduplicated by exact word-chain so write volume stays bounded regardless of traffic and the pool reflects genuinely distinct answers rather than N copies of whichever pairing most players land on — many board layouts only admit a handful of realistic short solutions in the first place, so the pool is naturally self-limiting on its own. Only catalog (dated) puzzles are ever stored, since a custom board's flattened-layout id is never looked up by the read side. `GET /api/solutions?date=YYYY-MM-DD` reads that pool back; the client fetches it lazily, only when the Yesterday modal actually opens (`modalManager.js`'s `openYesterdayModal` calls `loadPlayerSolutions`), and picks up to 2 random entries to display — never blocking or delaying the canonical solution shown above it, and simply staying hidden when nothing's stored yet for that date.
