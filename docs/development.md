...
**Ordering hazard, and the fix.** `gameEngine.applyBoardDefinition()` fires its own `onStateChange` synchronously the instant a fresh (blank) board is applied — before any restore replay gets a chance to run. Without a guard, that transient blank snapshot would race `saveCatalogProgressIfApplicable` and wipe out the very record about to be replayed. `app.js`'s `suppressProgressSave` flag (checked and set *before* `applyBoardDefinition` runs, cleared only after the restore replay completes) exists specifically for this — same shape as `puzzleReplay.js`'s own `suppressNextCompletionCelebration` flag for an analogous "don't let a transient event race the thing I'm about to do on purpose" problem.
